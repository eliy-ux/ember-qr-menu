...
import { db } from "./firebase.js?v=yoni-speed-70";
...
export function watchOrder(id, callback, onError) {
  return onSnapshot(trackingRef(id), snapshot => callback(snapshot.exists() ? { id: snapshot.id, ...snapshot.data() } : null), onError);
}

export function watchOrders(callback, onError) {
  return onSnapshot(query(collection(db, "orders"), orderBy("createdAt", "desc")), snapshot => callback(snapshot.docs.map(d => ({ id: d.id, ...d.data() }))), onError);
}
...
export function deleteOrder(id) {
  return Promise.all([deleteDoc(doc(db, "orders", id)), deleteDoc(trackingRef(id))]);
}

export async function createServiceRequest(request) {
  return addDoc(collection(db, "serviceRequests"), { ...request, status: "Pending", createdAt: serverTimestamp() });
}

export function watchServiceRequests(callback, onError) {
  return onSnapshot(query(collection(db, "serviceRequests"), orderBy("createdAt", "desc")), snapshot => callback(snapshot.docs.map(d => ({ id: d.id, ...d.data() }))), onError);
}

export function changeServiceRequestStatus(id, status) {
  return updateDoc(doc(db, "serviceRequests", id), { status });
}

export function watchMenu(callback, onError) {
  return onSnapshot(query(collection(db, "menu"), orderBy("name")), snapshot => callback(snapshot.docs.map(d => ({ ...d.data(), id: d.id }))), onError);
}

export function addMenuItem(item) {
  return addDoc(collection(db, "menu"), { ...item, createdAt: serverTimestamp() });
}

export function updateMenuItem(id, item) {
  return updateDoc(doc(db, "menu", id), item);
}
...
export async function getUserRole(email) {
  try {
    const snap = await getDoc(doc(db, "roles", String(email || "").toLowerCase()));
    return snap.exists() ? (snap.data().role || "staff") : "admin";
  } catch (e) {
